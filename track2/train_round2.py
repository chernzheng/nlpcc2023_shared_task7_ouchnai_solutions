...
def gen_train():
    with open("data/train.json", 'r') as f:
        ...
        for essay in essays:
            title = essay["Title"]
            paragraph_topic = ''.join(essay["ParagraphTopic"]).replace('?', '？')
            full_text_topic = essay["Full-textTopic"].replace('?', '？')
            labels = [0] * len(paragraph_topic)
            rex = re.search(full_text_topic, paragraph_topic)
            if rex:
                span = rex.span()
            ...
            yield {"ner_tags": labels, 'tokens': list(paragraph_topic), 'title': list(title)}

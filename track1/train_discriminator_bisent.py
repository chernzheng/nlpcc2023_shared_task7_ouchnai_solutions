    ...
    with open(input_dir, 'r', encoding='utf-8') as f:
        for line in f.readlines():
            ...
            for i in range(len(content) - 1):
                yield {'label': 1, 'text': content[i], 'text_pair': content[i+1]}
            # 负例
            if len(content) < 3:
                continue
            for i in range(len(content) - 1):
                delete_index = random.randint(0, 1)
                while True:
                    insert_index = random.randint(0, len(content)-1)
                    if insert_index not in [i, i+1]:
                        break
                if delete_index == 0:
                    yield {'label': 0, 'text': content[insert_index], 'text_pair': content[i+1]}
                elif delete_index == 1:
                    yield {'label': 0, 'text': content[i], 'text_pair': content[insert_index]}
